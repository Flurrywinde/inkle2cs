...
*** If a stitch has neither divert nor linkpath (i.e. end of story), make_story_array() seems to not put the text in. Then, make_cs_array() calls process_choices() on it despite there not being any. process_choices() should error check for this condition and abort. 
** Find other things besides diverts and linkpaths.
** Check print_r of $story_array to make sure everything is correct.
...
* \n weirdness not removed
* If divert target already proc'd, need to put a goto, not just add it to proc list
...
function process_story_element($elt_name, $level) {
	global $story_array, $cs_array;

	$this_elt = find_story_element_by_name($elt_name);
	dbug($elt_name."($level)", 'process_story_element(): Processing');
...
// Remove whitespace and then beginning and ending double-quotes from $str
...
//	dbug($st_array, 'Array of all stitches only name extracted:');
	...
	foreach($st_array as $stitch) {
		$stitch_name = $stitch[1]; // We got the name. So far so good.
		preg_match('/"((?:[^"]|\\")+)",\s+\{\s+("(divert|linkPath)": [^]]+)/s', $stitch[2], $contents);
		$stitch_text = remove_inkle_weirdness($contents[1]); // Now we have the text.
		...
		if(strcmp($contents[3], 'divert') == 0) {
			...
			foreach($next_block as $value) {
				...
				if(strcmp($value[3], 'null') != 0) $next_option = stripslashes(sanitize_str($value[3])); else $next_option = '';
				...
				$next_data[] = array(
					...
					'choice_text' => remove_inkle_weirdness($next_option),
		...
		'next_type' => $contents[3],			// 'divert' or 'linkPath' (i.e. tells if the block ends with a goto or choices)
...
global $story_array, $cs_array, $cs_text, $proc_list;
...
dbug('Success! Outputing to '.$output_type,'','',2);
if($output_type == 'file') {
...
function make_parents_array() {
	...
	foreach($story_array as $stitch) {
		if($stitch['next_type'] == 'divert') {
			...
			foreach($stitch['next_data'] as $choice) {
				if($choice['linkpath'] != '') {
...
<head>
    <meta charset="utf-8" />
    <title>Graphical Flowchart of Inkle Story</title>
</head>
<body>
...
TEST;
	$handle = init_output_file('inkle2cs.gv.html');
	file_write($handle, $html);
	fclose($handle);
	echo "<a href='inkle2cs.gv.html'>Graphical View of Story Stitches</a>";
}
